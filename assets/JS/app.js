...
let oldFarts = myEmployees.filter((element) =>{
  if(element.alder > 30){
    return element;
  }
})
console.table(oldFarts);
...
let youths = myEmployees.filter((x) =>{
  if(x.alder < 20){
    return x;
  }
})
console.table(youths);
...
let women = myEmployees.filter((x) =>{
  if(x.køn === "Kvinde" && x.alder > 30){
    return x;
  }
})
...
let nonBinary = myEmployees.filter((x) =>{
  if(x.køn === "Ikke-binær" && x.alder > 20)
  return x;
})

console.table(nonBinary);
